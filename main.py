...
query_cols = []
query_values = ['NY'] #Change to be the value(s) you're querying in the column you've specified
...
def make_query(cols, values, limit):
    ...
    query = "SELECT " + string_cols + " FROM " + TABLE_ID + " WHERE STABBR = '" + string_values + "'"
...
def index():
    ...
    res = get_all_data(make_query(query_cols, query_values, 1000)) 
